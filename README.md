...
进入后输入昵称登录（可重名，身份以会话为准）。同一账号新连接会顶掉旧连接。大厅按 `o` 可退出登录并换用户名。会话在无活动（含在线心跳）24 小时后删除，并解散该会话创建的房间。

| 场景 | 按键 |
|------|------|
| 大厅 | `1` 麻将三人机 / `2` 十点半通比 / `3` 打庄 / `j` 加入（Tab 补全房间号） / `r` 刷新 / `o` 退出登录 |
| 房间 | `c` 配置 / `y` `u` 准备 / `b` 加人机 / `d` 移除人机 / `t` 转让房主 / `k` 踢人 / `s` 开始 / `Enter` 聊天 / `l` 离开 / `o` 退出登录 |
| 麻将 | `←` `→` 选牌，空格锁定再出牌；`p` 碰 `g` 明杠 `h` 胡 `a` 暗杠 `b` 补杠 `n` 过；`Enter` 聊天 `Esc` 退出聊天 |
| 十点半 | `h` 要牌 `s` 停牌；`Enter` 聊天 `Esc` 退出聊天 |
| 全局 | `q` 退出 |
...
| `SESSION_TTL_MS` | `86400000` | 会话空闲超时：无活动 24 小时后删除会话及其创建的房间 |
