...
玩家用 npm 安装客户端，连到你部署的服务器即可。当前客户端 npm 包 `@yifunxxx/qipai-cli` 版本：**0.1.5**。
...
| 场景 | 按键 |
|------|------|
| 大厅 | `1` 麻将三人机 / `2` 十点半通比 / `3` 打庄 / `j` 加入 / `r` 刷新 / `o` 退出登录 |
| 房间 | `c` 配置 / `y` `u` 准备 / `b` 加人机 / `d` 移除人机 / `t` 转让房主 / `k` 踢人 / `s` 开始 / `l` 离开 / `o` 退出登录 |
